...
class Story(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT)
    created_at = models.DateTimeField(auto_now_add=True)
    file = models.FileField(upload_to='content/', validators=[validate_file_extension])
...
class StoryContent(models.Model):
    ...
    story = models.ForeignKey(Story, related_name='story_content', on_delete=models.CASCADE)
    typeContent = models.CharField(max_length=10, choices=STORY_CONTENT_CHOICES)
    file = models.FileField(upload_to='post_content/') 
...
class Mention(models.Model):
    ...
    def clean(self):
        if self.user == self.post.user:
            raise ValidationError("You cannot mention yourself in your own post.")
...
class Reaction(models.Model):
    ...
    def __str__(self):
        return f'{self.user.username} reacted {self.status} to post "{self.post.caption}"'
